...
export const Map: FC<MapProps> = memo(
	({ title, zoom = 14, coordinates }) => {
		...
		const handleMapLoad = useCallback(
			(
				element: NonNullable<RefObject<HTMLDivElement>['current']>,
	...
	(prevProps, nextProps) => {
		// no props given, prevent update
		if (!prevProps || !nextProps || !prevProps.coordinates || !nextProps.coordinates) {
			return false;
		}

		// no real data changes, skip update
		if (
